#!/bin/bash

# Deployment script for joshgold.design via Namecheap cPanel FTP
# This script builds the Next.js app and uploads it via FTP

set -e

echo "🚀 Starting deployment to joshgold.design..."

# Build the Next.js app (if npm is available)
if command -v npm &> /dev/null; then
    echo "📦 Building Next.js app..."
    if ! npm run build; then
        echo "❌ Build failed. Please check for errors."
        exit 1
    fi
    echo "✅ Build complete!"
else
    echo "⚠️  npm not found. Skipping build step."
    echo "   Please run 'npm run build' manually if you haven't already."
fi

# Check if out directory exists
if [ ! -d "out" ]; then
    echo "❌ Error: 'out' directory not found."
    echo "   Please run 'npm run build' first to create the build files."
    exit 1
fi

# Use Python FTP script for upload
if command -v python3 &> /dev/null; then
    echo "📤 Uploading via FTP..."
    python3 deploy-ftp.py
else
    echo "❌ Python 3 not found. Cannot upload files."
    echo "   Please install Python 3 or use an FTP client manually."
    exit 1
fi
